...
public abstract class BaseActivity extends AppCompatActivity implements CustomAdapt, OnTitleBarListener {
    ...
    /**
     * 检查并应用主题
     * 确保每个Activity都使用正确的主题
     */
    private void checkAndApplyTheme() {
        try {
            // 检查主题是否需要更新
            boolean themeChanged = com.github.tvbox.osc.util.ThemeManager.getInstance().checkThemeUpdate();
            if (themeChanged) {
                android.util.Log.d("BaseActivity", "主题已更新: " + this.getClass().getSimpleName());
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void onConfigurationChanged(android.content.res.Configuration newConfig) {
        ...
        // 通知主题管理器配置变化
        com.github.tvbox.osc.util.ThemeManager.getInstance().onConfigurationChanged(newConfig);
        ...
        if (themeTag == 0) {
            // 重新初始化状态栏
            initStatusBar();
            android.util.Log.d("BaseActivity", "系统主题变化，重新初始化状态栏: " + this.getClass().getSimpleName());
        }
    }
    ...
    protected void onDestroy() {
        // 预防内存泄漏
        com.github.tvbox.osc.util.MemoryLeakPreventer.cleanupActivity(this);
