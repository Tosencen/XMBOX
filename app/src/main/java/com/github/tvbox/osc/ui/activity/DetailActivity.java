...
public class DetailActivity extends BaseVbActivity<ActivityDetailBinding> {
    ...
    private void initView() {
        ...
        mBinding.mGridViewFlag.setLayoutManager(new LinearLayoutManager(this, LinearLayoutManager.HORIZONTAL, false));
        ...
        seriesFlagAdapter.setOnItemClickListener((adapter, view, position) -> {
            android.util.Log.d("DetailActivity", "线路按钮被点击 - position: " + position);
            FastClickCheckUtil.check(view);
            chooseFlag(position);
        });

        // 添加额外的触摸监听器作为备用方案
        mBinding.mGridViewFlag.addOnItemTouchListener(new androidx.recyclerview.widget.RecyclerView.SimpleOnItemTouchListener() {
            @Override
            public boolean onInterceptTouchEvent(androidx.recyclerview.widget.RecyclerView rv, android.view.MotionEvent e) {
                if (e.getAction() == android.view.MotionEvent.ACTION_UP) {
                    ...
                    if (child != null) {
                        ...
                        if (position >= 0 && position < seriesFlagAdapter.getItemCount()) {
                            android.util.Log.d("DetailActivity", "触摸监听器检测到点击 - position: " + position);
                            chooseFlag(position);
                            return true;
                        }
                    }
...
//                    insertVod(sourceKey, vodInfo);
    ...
    private void chooseFlag(int position) {
        android.util.Log.d("DetailActivity", "chooseFlag被调用 - position: " + position);
        //新选中的flag
        String newFlag = seriesFlagAdapter.getData().get(position).name;
        android.util.Log.d("DetailActivity", "新选中的线路: " + newFlag + ", 当前线路: " + (vodInfo != null ? vodInfo.playFlag : "null"));
        if (vodInfo != null && !vodInfo.playFlag.equals(newFlag)) {
            ...
            if (currentSeriesList != null && currentSeriesList.size() > vodInfo.playIndex) {//有效集数
            ...
            android.util.Log.d("DetailActivity", "线路已切换从 " + oldFlag + " 到 " + newFlag);

            // 刷新剧集列表
            refreshList();

            // 重要：触发视频重新播放（无论是否预览模式）
            android.util.Log.d("DetailActivity", "触发视频重新播放，showPreview: " + showPreview);
            jumpToPlay();
        }
    }
