...
class SettingActivity : BaseVbActivity<ActivitySettingBinding>() {
    ...
    private var homeRec = Hawk.get(HawkConfig.HOME_REC, 2)
    private var dnsOpt = Hawk.get(HawkConfig.DOH_URL, 1)
    ...
    override fun init() {
        ...
        mBinding.tvHomeRec.text = getHomeRecName(Hawk.get(HawkConfig.HOME_REC, 2))
        ...
        mBinding.llHomeRec.setOnClickListener { v: View? ->
            FastClickCheckUtil.check(v)
            val defaultPos = Hawk.get(HawkConfig.HOME_REC, 2)
            ...
            dialog.setAdapter(object : SelectDialogInterface<Int?> {
                override fun click(value: Int?, pos: Int) {
                    ...
                    mBinding.tvHomeRec.text = getHomeRecName(value?:2)
                }

                override fun getDisplay(value: Int?): String {
                    return getHomeRecName(value?:2)
                }
            }, object : DiffUtil.ItemCallback<Int>() {
                override fun areItemsTheSame(oldItem: Int, newItem: Int): Boolean {
                    return oldItem == newItem
        ...
        mBinding.llTheme.setOnClickListener(View.OnClickListener { view: View? ->
            ...
            dialog.setOnDismissListener { dialog1: DialogInterface? ->
                if (oldTheme != Hawk.get(HawkConfig.THEME_TAG, 0)) {
                    // 使用ThemeManager强制刷新主题
                    com.github.tvbox.osc.util.ThemeManager.getInstance().forceRefreshTheme()
                    val bundle = Bundle()
    ...
    override fun onBackPressed() {
        if (homeRec != Hawk.get(HawkConfig.HOME_REC, 2) || dnsOpt != Hawk.get(
                HawkConfig.DOH_URL,
                1
            )
